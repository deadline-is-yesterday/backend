...
3. **Стена (< 0)**: статичный барьер — значение не меняется никогда.
   Огонь не проходит через стену и не разрушает её.
...
class FireSystem:
    ...
    def update(self) -> bool:
        ...
        2. Распространение огня: для каждой клетки пересчитывается температура.
           Стены — статичные барьеры, их значение не меняется.
        3. Очистка: потухшие источники удаляются.
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                # --- Стена: статичный барьер, не разрушается ---
                if current < 0:
